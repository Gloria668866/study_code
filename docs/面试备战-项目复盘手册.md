...
### 3.1.1 多轮 Query Rewrite（追问改写）

**为什么需要 Query Rewrite**（面试高频）：
> 多轮对话中用户会省略主语/实体——"哪个月卖的最好""为什么下降了"。旧方案用二元追问检测（`_detect_followup`），只能判断"是否追问"，但追问本身仍可能是 sql/rag 意图。正确做法是**先改写后分类**：把省略问题补全为独立问题，再让 NLU 正常分类。

**架构位置**：
```
用户消息 + 历史 → [Query Rewrite] → 独立完整问题 → [5层NLU] → 意图路由
                      ↓ is_meta=true
                   直接走 chat（纯元信息追问）
```

**is_meta 判定标准**（容易被追问）：
> 只有"不需要查数据库/检索文档就能回答"的问题才是 meta。例如"这是几几年的""数据来源是哪"。而"为什么下降了""哪个月最好"都需要查数据或检索文档，不是 meta。

**改写 prompt 核心四规则**：
1. 补全省略实体（从历史中找被省略的主语/宾语）
2. 解析代词（"它""那个"→具体实体）
3. 保留新维度（月份/趋势/对比/口碑）
4. 已完整问题原样返回

**代码位置**：`app/graph.py:_rewrite_query()` / `_REWRITE_SYS`

**面试追问「参考了哪些项目的做法」**：
> 参考 DB-GPT（多轮对话管理）、CoE-SQL（query decomposition）、STAR（self-taught reasoner）的思路。核心共识是：多轮问题必须在进入 NLU pipeline 之前就被改写为单轮问题，而不是在 NLU 内部特殊处理多轮逻辑——后者会把多轮耦合进每个下游模块，维护成本指数增长。
...
### Bug 9：多轮追问被误判为 chat（Query Rewrite 改造）【P0】
**现象**：用户问"哪吒汽车25年卖的咋样"→正确走 sql，紧接着问"哪个月份卖的最好呢"→被分类为 chat，返回问候语
**根因**：旧版 `_detect_followup()` 是二元门控（是/否追问），一旦判定为追问就直接走 chat 意图，完全绕过 NLU 分类。实际上"哪个月份卖的最好"是需要查数据库的新问题，只是省略了主语
**修复**：删除 `_detect_followup()` 二元门控，替换为企业级 Query Rewrite 架构：`user msg + history → [LLM 改写] → 独立完整问题 → [NLU 分类]`。只有纯元信息追问（"这是几几年的数据""怎么算的"）才走 chat
**面试说法**：多轮对话的核心难题不是"检测是否追问"，而是"把省略/代词补全为独立问题"。Query Rewrite 是业界标准做法（DB-GPT/CoE-SQL/STAR 等项目都用），核心思路是把多轮问题转为单轮问题，然后复用已有的单轮 NLU pipeline
...
| Windows `__pycache__` 提供过期字节码 | 修改 `graph.py` 后 uvicorn --reload 不生效，旧逻辑仍在执行 | `rm -rf app/__pycache__/` 后重启 uvicorn | Windows 文件锁可阻止 .pyc 被覆盖，uvicorn reload 只看 .py mtime 不一定清 .pyc |
| SSE 端点 vs 调试端点行为不一致 | `/api/ask`（SSE）正确传 history，`/api/ask_sync` 未传 history 导致改写无效 | `ask_sync` 加上 `history = _load_history(db, user, body.conversation_id)` | 多端点维护时必须检查每个入口的参数完整性，debug 端点容易被遗忘 |
| 多轮改写 is_meta 过度触发 | "为什么销量下降"被判为 is_meta（元信息追问），实际应改写为完整问题查 RAG | 精炼 `_REWRITE_SYS` prompt，明确 is_meta 只适用于"不需要查数据就能回答"的场景 | LLM 分类边界模糊时，prompt 中反面示例比正面定义更有效 |
...
#### Q5: 多轮对话怎么处理的？为什么不用 LangGraph checkpointer？

**30秒核心答**：
两层设计：①Query Rewrite（`_rewrite_query`）——用 LLM 将省略/代词问题改写为独立完整问题，再进入正常 NLU pipeline；②`active_entities` 跨轮实体累积——NLU 抽取的实体合并到全局 state，后续问题即使不被改写也能继承上下文实体。不用 checkpointer 是因为本项目的多轮是「对话级」的（用户发新消息触发新一轮图执行），不是「图内多轮」（同一图内等待人类输入）。checkpointer 适合 Human-in-the-loop 的槽位填充场景，引入到本项目反而增加复杂度。

**追问「改写有什么坑？」**：
> 三个坑：①`is_meta` 判定边界模糊——"为什么下降了"看起来像追问但实际需要查 RAG，prompt 中必须明确"不需要查数据就能回答"才是 meta；②Windows `__pycache__` 缓存导致代码修改不生效——uvicorn reload 不清 .pyc，必须手动删 `__pycache__/`；③多端点一致性——SSE 端点正确传 history 但 debug 端点 `ask_sync` 遗漏了 history 参数，导致改写功能看起来"时灵时不灵"。

**追问「参考了哪些业界方案？」**：
> DB-GPT 的多轮对话管理、CoE-SQL 的 query decomposition、STAR 的 self-taught reasoning。核心共识：多轮改写必须在 NLU pipeline 之前完成（pre-processing），不能在 NLU 内部用特殊规则处理——否则每个下游模块都要感知多轮，耦合度指数增长。
...
**30秒核心答**：
三层策略递进：①搜索引擎 snippet 优先（百度/必应搜索结果的摘要本身就含销量数据，零反爬风险）；②普通新闻站用 httpx 直连（搜狐汽车/网易汽车无反爬）；③反爬站点（懂车帝/汽车之家/乘联会）用 Playwright 隐身浏览器——手动配置反检测参数（`disable-blink-features=AutomationControlled`、随机 UA、navigator.webdriver 重写、假 plugins 数组），模拟真实浏览器行为。如果 crawl4ai 可用则使用其完整反爬栈（Stealth/Undetected 适配器 + 反爬检测 + 代理轮换 + 指数退避）。

**追问「具体做了哪些反检测」**：
> 五个层面：①Chromium 启动参数（`--disable-blink-features=AutomationControlled` 去掉自动化标记）；②JS 注入重写 `navigator.webdriver=undefined`（最关键的检测信号）；③假装有 5 个 plugins（headless 默认 0 个，是指纹特征）；④随机 UA 轮换（5 个真实 Chrome UA）；⑤等待 JS 渲染完成再抓取（SPA 站点必须等 DOM 就绪）。

**追问「为什么不直接用 Selenium？」**：
> Playwright 比 Selenium 快（原生 CDP 协议 vs HTTP 转发）、API 更现代（async/await 原生支持）、自带 Chromium 不依赖本地 Chrome 版本。且 Playwright 社区有成熟的 stealth 插件（playwright-stealth），Selenium 的 undetected-chromedriver 维护不如 patchright。
