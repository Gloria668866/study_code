...
async def extract_interview_topic_node(state: AgentState):
    ...
    """
    await update_mysql("开始抽取面试题", record_id=state["record_id"], processing_status=1)
    ...
    for i, chunk in enumerate(text_chunks):
        ...
        # 流式输出当前块的处理结果
        collected_text = ""
        for chunk_result in chain.stream({"voice_text": chunk}):
        ...
        try:
            ...
            await update_mysql(f"第 {i + 1} 块解析失败: {e}", record_id=state["record_id"])
            continue
