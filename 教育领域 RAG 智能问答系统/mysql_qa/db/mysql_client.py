...
class MySQLClient:
    ...
    def insert_data(self, csv_path):
        try:
            ...
            for _, row in data.iterrows():
                insert_query = "INSERT INTO subjects_table (subject_name, question, answer) VALUES (%s, %s, %s)"
    ...
    def fetch_questions(self):
        ...
        try:
            # 执行查询
            self.cursor.execute("SELECT question FROM subjects_table")
            # 获取结果
    ...
    def fetch_answer(self, question):
        ...
        try:
            # 执行查询
            self.cursor.execute("SELECT answer FROM subjects_table WHERE question=%s", (question,))
            # 获取结果
...
if __name__ == '__main__':
    ...
    #mysqlclient.insert_data('../data/JP学科知识问答.csv')
