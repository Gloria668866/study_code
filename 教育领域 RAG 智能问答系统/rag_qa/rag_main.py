...
def main(query_mode=True, directory_path="data"):
    ...
    #   初始化 DeepSeek API 客户端 (通过 OpenAI 接口)
    #   确保 DEEPSEEK_API_KEY 和 DEEPSEEK_BASE_URL 已在 config.ini 中配置
    try:

        client = OpenAI(api_key=conf.DEEPSEEK_API_KEY,
    ...
    # 定义 LLM 调用函数（流式输出，适配 RAGSystem 的 streaming 和 collect 两种消费模式）
    def call_llm(prompt):
        if not client:
