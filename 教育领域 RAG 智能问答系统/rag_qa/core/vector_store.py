...
class VectorStore:
    ...
    def __init__(self,
        ...
        self.reranker = CrossEncoder(r"D:\AI_space_code\FunctionCall_study\rag_xiangmu\edu_rag_system\rag_qa\models\bge-reranker-large",device=self.device)
        # 初始化 BGE-M3 嵌入函数，使用 float32 与 Milvus 集合类型一致
        self.embedding_function = BGEM3EmbeddingFunction(
            model_name_or_path=r'D:\AI_space_code\FunctionCall_study\rag_xiangmu\edu_rag_system\rag_qa\models\bge-m3',
            use_fp16=False,
            device=self.device
        )
        # 获取稠密向量的维度
...
# 定义方法，向向量存储添加文档
    def add_documents(self, documents):
        ...
        for i, doc in enumerate(documents):
            ...
            sparse_vector = {}
            # 获取第i行对应的稀疏向量数据
            # scipy>=1.12 不支持 1D 稀疏切片，用 2D 索引 [[i]] + tocoo()
            row = embeddings["sparse"][[i]].tocoo()
            indics = row.col
            values = row.data
            # 将索引和值进行配对，存储到字典中
    ...
    def hybrid_search_with_rerank(self, query, k=conf.RETRIEVAL_K, source_filter=None):
        ...
        sparse_query_vector = {}
        # 获取查询稀疏向量的第 0 行数据
        # scipy>=1.12 不支持 1D 稀疏切片，用 2D 索引 [[0]] + tocoo()
        row = query_embeddings["sparse"][[0]].tocoo()
        indices = row.col
        values = row.data
        # 将索引和值配对，填充稀疏向量字典
        ...
        sub_chunks = []
        for hit in results:
            doc = self._doc_from_hit(hit["entity"], hit_id=self._hit_value(hit, "id"))
            score = self._hit_value(hit, "distance")
            if score is None:
                score = self._hit_value(hit, "score")
        ...
        if len(parent_docs) < 2:
            for doc in parent_docs:
                doc.metadata.setdefault("rerank_score", None)
        ...
        if parent_docs:
            ...
            # 根据得分从高到低排序文档
            ranked_pairs = sorted(zip(scores, parent_docs), key=lambda item: item[0], reverse=True)
            ranked_parent_docs = []
    ...
    def _get_unique_parent_docs(self, sub_chunks):
        ...
        for chunk in sub_chunks:
            ...
            if parent_content and parent_content not in parent_contents:
                ...
                metadata["child_ids"] = [chunk.metadata.get("id")]
                unique_doc = Document(page_content=parent_content, metadata=metadata)
                unique_docs.append(unique_doc)
                # 将父块内容添加到去重集合
                parent_contents[parent_content] = unique_doc
            elif parent_content:
                existing = parent_contents[parent_content]
                child_id = chunk.metadata.get("id")
                if child_id:
                    existing.metadata.setdefault("child_ids", []).append(child_id)
                current_score = chunk.metadata.get("hybrid_score")
                previous_score = existing.metadata.get("hybrid_score")
                if current_score is not None and (previous_score is None or current_score > previous_score):
                    existing.metadata["hybrid_score"] = current_score
        # 返回去重后的父文档列表
        return unique_docs


    # 定义私有方法，从 Milvus 查询结果创建 Document 对象
    def _hit_value(self, hit, key):
        try:
            if hasattr(hit, "get"):
                return hit.get(key)
            return hit[key]
    ...
    def _doc_from_hit(self, hit, hit_id=None):
        # 创建并返回 Document 对象，填充内容和元数据
        return Document(
            page_content=hit.get("text"),
            metadata={
                "id": hit.get("id") or hit_id,
...
if __name__ == '__main__':
    vstore = VectorStore()
    directory_path = r'D:\AI_space_code\FunctionCall_study\rag_xiangmu\edu_rag_system\rag_qa\data\ai_data'
