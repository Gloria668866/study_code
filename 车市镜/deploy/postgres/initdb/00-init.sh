...
# 用 app_rw 自己建业务表，使后续 SQLAlchemy 轻量迁移具备 ALTER 权限；
# 仅扩展安装保留超级用户执行。
PGPASSWORD="$APP_DB_PASSWORD" psql -v ON_ERROR_STOP=1 \
  --username "$APP_DB_USER" --dbname "$APP_DB_NAME" \
  -f "$SQL_DIR/30-app-schema.sql"
...
-- 业务表由 ${APP_DB_USER} 自己创建并拥有；保留下列默认权限供该角色未来建表。
ALTER DEFAULT PRIVILEGES FOR ROLE ${APP_DB_USER} IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO ${APP_DB_USER};
ALTER DEFAULT PRIVILEGES FOR ROLE ${APP_DB_USER} IN SCHEMA public GRANT USAGE, SELECT ON SEQUENCES TO ${APP_DB_USER};
EOSQL
